<template>
  <div id="main">
    <div class="store-content item">
      <div class="item-box" v-if="itemInfo">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li :class="{on: index === imgIndex}" v-for="(img, index) in itemInfo.ali_images" :key="index" @click="tableImg(index)"><img :src="img + '?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
            <div class="thumb">
              <ul>
          ...
          <div class="sku-dynamic-params-panel">
            <div class="sku-dynamic-params clear">
              ...
              <ul class="params-colors">
                <li :class="{'cur': color.id === itemId}" v-for="(color, index) in itemInfo.sku_list" :key="index">
                  <router-link :title="color.color" :to="{name: 'Item', query: {itemId: color.id}}">
                    <img :src="color.image + '?x-oss-process=image/resize,w_52'">
                  </router-link>
                </li>
              </ul>
            </div>
...
export default {
  ...
  computed: {
    itemInfo () {
      return this.itemsData.find(item => Number(this.itemId) === Number(item.sku_id))
    }
  },
  watch: {
    '$route.query.itemId' () {
